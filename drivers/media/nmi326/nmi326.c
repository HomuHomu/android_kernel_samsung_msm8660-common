...
static int isdbt_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	...
	ret1 = copy_to_user(buf, pdev->rwBuf, count); //shubham
...
static int isdbt_write(struct file *filp,const char *buf, size_t count, loff_t *f_pos) //shubham...aded const
{
	int rv = 0;
	ISDBT_OPEN_INFO_T* pdev = (ISDBT_OPEN_INFO_T*)(filp->private_data);
	...
	ret1 = copy_from_user(pdev->rwBuf, buf, count);
...
static long isdbt_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) //shubham....replaced int with long
{
	long res = 1;
	ISDBT_OPEN_INFO_T* pdev = (ISDBT_OPEN_INFO_T*)(filp->private_data);

	int	err=0, size; //shubham...initialising err
	...
	switch(cmd)
	{
		...
		case IOCTL_ISDBT_INTERRUPT_REGISTER:
		{
			...
			if(pdev->irq_status == DTV_IRQ_DEINIT)
			{
				...
				if(request_irq(gpio_cfg.irq, isdbt_irq_handler, /*IRQF_DISABLED|*/IRQF_TRIGGER_LOW, ISDBT_DEV_NAME, (void*)pdev) < 0) {
